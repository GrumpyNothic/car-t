...
int main(void) {
    ...
    do Paciente A (P1-CLL) do poster do SIMMA (2024)
    */
    ...
    double C_D_inicial = 1.13e8, C_Dose2 = 3.39e8, C_Dose3 = 6.78e8, C_D, C_T = 0.0, C_M = 0.0;
    ...
    double g0 = 5e-2, p = 0, q = 1e4;
    double var1 = 0, var2 = 0;

    FILE *arq, *d1, *d2, *d3, *d4, *d5, *d6, *d7, *d8;

    arq = fopen("pa2_dados.txt", "w");
    d1 = fopen("pa2_dados_tumor_t0.txt", "w");
    d2 = fopen("pa2_dados_tumor_t30.txt", "w");
    d3 = fopen("pa2_dados_tumor_t60.txt", "w");
    d4 = fopen("pa2_dados_tumor_t90.txt", "w");
    d5 = fopen("pa2_dados_tumor_t180.txt", "w");
    d6 = fopen("pa2_dados_heat_taxa.txt", "w");
    d7 = fopen("pa2_dados_heat_efeito.txt", "w");
    d8 = fopen("pa2_dados_barplot.txt", "w");
	...
	rsc1 = rc1 + sc1;
        ...
        for (i=0; i<n+2; i++){
            k13[i] = r*T1[i]*(1-b*rsc1) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D1+C_T1) / (a+C_D1+C_T1+d*sc1) ) *T1[i]; //tum
	...
	rsc2 = rc2 + sc2;
        ...
        for (i=0; i<n+2; i++){
            k23[i] = r*T2[i]*(1-b*rsc2) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D2+C_T2) / (a+C_D2+C_T2+d*sc2) ) *T2[i]; //tum
	...
	rsc3 = rc3 + sc3;
        ...
        for (i=0; i<n+2; i++){
            k33[i] = r*T3[i]*(1-b*rsc3) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D3+C_T3) / (a+C_D3+C_T3+d*sc3) ) *T3[i]; //tum
	...
	rsc4 = rc4 + sc4;
        ...
        for (i=0; i<n+2; i++){
            k43[i] = r*T4[i]*(1-b*rsc4) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D4+C_T4) / (a+C_D4+C_T4+d*sc4) ) *T4[i]; // sensiveis
