...
int main(void) {
    ...
    double beta = 9.1921e-1, eta = 1.0e-8;
    double rmin = 3.03, p1 = 1.050917958e1, p2, p3 = 8.5e1, A = 8.1e4, xi = 3.09366e-1, theta = 6.0e-6, alpha = 5.5e-7;
    double epsilon = 1.7e-4, mu=4.758e-5;
    double lambda = 3.0e-2, delta = 8.5366e-2;
    double r = 0.176, b=0.5e-12, gamma = 2.25, d= 3.05e-1, a = 1.0e3;
 
    double dt = 1e-6;
    double tk = 0;

    double C_D1, C_D2, C_D3, C_D4, C_T1, C_T2, C_T3, C_T4, C_M1, C_M2, C_M3, C_M4, C_E1, C_E2, C_E3, C_E4;
    double kt, Ce, prol, cit, dist, citt, func, func1, func2, cmest, cest, incl1, incl2, incl3, incl4, incl5, incl6;
    double k10, k20, k30, k40, k11, k12, k13, k14[2001], k21, k22, k23, k24[2001], k31, k32, k33, k34[2001], k41, k42, k43, k44[2001];
    double C_D_inicial = 1.13e8, C_Dose2 = 3.39e8, C_Dose3 = 6.78e8, C_D, C_T = 0.0, C_M = 0.0, C_E = 0.0;
    ...
    FILE *arq, *d1, *d2, *d3, *d4, *d5, *d6, *d7, *d8, *d9, *d10, *d11;

    arq = fopen("pa1_dados.txt", "w");
    d1 = fopen("pa1_dados_tumor_t0.txt", "w");
    d2 = fopen("pa1_dados_tumor_t30.txt", "w");
    d3 = fopen("pa1_dados_tumor_t60.txt", "w");
    d4 = fopen("pa1_dados_tumor_t90.txt", "w");
    d5 = fopen("pa1_dados_tumor_t180.txt", "w");
    d9 = fopen("pa1_dados_tumor_t360.txt", "w");
    d10 = fopen("pa1_dados_tumor_t540.txt", "w");
    d11 = fopen("pa1_dados_tumor_t1590.txt", "w");
    d6 = fopen("pa1_dados_heat_taxa.txt", "w");
    d7 = fopen("pa1_dados_heat_efeito.txt", "w");
    d8 = fopen("pa1_dados_barplot.txt", "w");
    ...
    for(i=0; i<n+2; i++){
        ...
        if (xises[i] <= xbar){ // resistentes
            aux[i] = 0/(sd*sqrt(2*M_PI)) * exp( -0.5 * pow((xises[i]-xn)/sd, 2));
            T[i] = 0;
        }
        else{ // sensiveis
            aux[i] = 0;
    ...
    fprintf(arq, "%e %e %e %e %e %e %e %e %e %e %e %e\n", tk, C_D, C_T, C_M, C_D+C_T+C_M+C_E, sc, rc, rsc, var1, var2, kt, rbar);
	...
	rsc1 = rc1 + sc1;
        ...
        k13 = lambda*C_T1 - delta*C_E1; //exaust
        for (i=0; i<n+2; i++){
            k14[i] = r*T1[i]*(1-b*rsc1) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D1+C_T1) / (a+C_D1+C_T1+d*sc1) ) *T1[i]; //tum
            if (fabs(k14[i]) < 1e-16){ k14[i] = 0; }
        }
        ...
        C_E2 = C_E + 0.5*dt*k13;
        if (C_E2 < 1e-16){ C_E2 = 0; }
    	for (i=0; i<n+2; i++){
            T2[i] = T[i]+ 0.5*dt*k14[i];
	...
	rsc2 = rc2 + sc2;
        ...
        k23 = lambda*C_T2 - delta*C_E2; //exaust
        for (i=0; i<n+2; i++){
            k24[i] = r*T2[i]*(1-b*rsc2) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D2+C_T2) / (a+C_D2+C_T2+d*sc2) ) *T2[i]; //tum
            if (fabs(k24[i]) < 1e-16){ k24[i] = 0; }
        }
        ...
        C_E3 = C_E + 0.5*dt*k23;
        if (C_E3 < 1e-16){ C_E3 = 0; }
    	for (i=0; i<n+2; i++){
            T3[i] = T[i]+ 0.5*dt*k24[i];
	...
	rsc3 = rc3 + sc3;
        ...
        k33 = lambda*C_T3 - delta*C_E3; 
        for (i=0; i<n+2; i++){
            k34[i] = r*T3[i]*(1-b*rsc3) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D3+C_T3) / (a+C_D3+C_T3+d*sc3) ) *T3[i]; //tum
            if (fabs(k34[i]) < 1e-16){ k34[i] = 0; }
        }
	...
	rsc4 = rc4 + sc4;
        ...
        k43 = lambda*C_T4 - delta*C_E4; //exaust
        for (i=0; i<n+2; i++){
            k44[i] = r*T4[i]*(1-b*rsc4) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D4+C_T4) / (a+C_D4+C_T4+d*sc4) ) *T4[i]; // sensiveis
            if (fabs(k44[i]) < 1e-16){ k44[i] = 0; }
        }
    	...
    	if (C_M < 1e-16){ C_M = 0; }
        C_E = C_E + (dt/6.0)*(k13 + 2.0*k23 + 2.0*k33 + k43);
        if (C_E < 1e-16){ C_E = 0; }
	for (i=0; i<n+2; i++){
	    T[i] = T[i] + (dt/6.0) * (k14[i] + 2.0*k24[i] + 2.0*k34[i] + k44[i]);
	    if (T[i] < 1e-16){ T[i] = 0; }
	}
	...
	rsc = rc + sc;
        ...
        // Salvamento dos dados dia 180
        if (fabs(tk - 180.0) < 1e-6){
            fprintf(d8, "%.0f\t%e\t%e\t%e\n", tk, sc, rc, rsc); // Barplot t90
            for (i=0; i<n+2; i++){ // salvando a distribuição do tumor
                fprintf(d5, "%e\t%e\n", xises[i], T[i]);
            }
        ...
        // Salvamento dos dados dia 360
        if (fabs(tk - 360.0) < 1e-6){
            fprintf(d8, "%.0f\t%e\t%e\t%e\n", tk, sc, rc, rsc); // Barplot t90
            for (i=0; i<n+2; i++){ // salvando a distribuição do tumor
                fprintf(d9, "%e\t%e\n", xises[i], T[i]);
            }
        ...
        // Salvamento dos dados dia 540
        if (fabs(tk - 540.0) < 1e-6){
            fprintf(d8, "%.0f\t%e\t%e\t%e\n", tk, sc, rc, rsc); // Barplot t90
            for (i=0; i<n+2; i++){ // salvando a distribuição do tumor
                fprintf(d10, "%e\t%e\n", xises[i], T[i]);
            }
        ...
        //fprintf(arq, "%e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e\n", tk, C_D, C_T, C_M, C_E, T, C_D+C_T+C_M+C_E, kt, prol, citt, dist, func, func1, func2, incl1, incl2, incl3, incl4, incl5, incl6);
        cont++;
