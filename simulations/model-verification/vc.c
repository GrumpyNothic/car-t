...
int main(void) {
    ...
    double epsilon = 1.0e-4, mu = 4.0e-2;
    double lambda = 8.0e-1, delta = 1.573;
    double r = 0.176, b=0.5e-12, gamma = 3.69e-1, d= 3.05e-1, a = 1.0e4;
    ...
    double C_D_inicial = 4.2e7, C_Dose2 = 0, C_Dose3 = 0, C_D, C_T = 0.0, C_M = 0.0, C_E = 0.0;
    ...
    arq = fopen("pc1_dados.txt", "w");
    d1 = fopen("pc1_dados_tumor_t0.txt", "w");
    d2 = fopen("pc1_dados_tumor_t30.txt", "w");
    d3 = fopen("pc1_dados_tumor_t60.txt", "w");
    d4 = fopen("pc1_dados_tumor_t90.txt", "w");
    d5 = fopen("pc1_dados_tumor_t180.txt", "w");
    d6 = fopen("pc1_dados_heat_taxa.txt", "w");
    d7 = fopen("pc1_dados_heat_efeito.txt", "w");
    d8 = fopen("pc1_dados_barplot.txt", "w");
	...
	rsc1 = rc1 + sc1;
        ...
        for (i=0; i<n+2; i++){
            k14[i] = r*T1[i]*(1-b*rsc1)*(1-((p+q)/(rsc1+q))) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D1+C_T1) / (a+C_D1+C_T1+d*sc1) ) *T1[i]; //tum
	...
	rsc2 = rc2 + sc2;
        ...
        for (i=0; i<n+2; i++){
            k24[i] = r*T2[i]*(1-b*rsc2)*(1-((p+q)/(rsc2+q))) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D2+C_T2) / (a+C_D2+C_T2+d*sc2) ) *T2[i]; //tum
	...
	rsc3 = rc3 + sc3;
        ...
        for (i=0; i<n+2; i++){
            k34[i] = r*T3[i]*(1-b*rsc3)*(1-((p+q)/(rsc3+q))) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D3+C_T3) / (a+C_D3+C_T3+d*sc3) ) *T3[i]; //tum
	...
	rsc4 = rc4 + sc4;
        ...
        for (i=0; i<n+2; i++){
            k44[i] = r*T4[i]*(1-b*rsc4)*(1-((p+q)/(rsc4+q))) - ( (1-gx[i])*g0 + gamma*gx[i] ) *( (C_D4+C_T4) / (a+C_D4+C_T4+d*sc4) ) *T4[i]; // sensiveis
